...
export default function MyModal({title, urls, openModalProj, status, images, urlWeb}){
  ...
  function prevImage(){
    ...
    actualImg > 0 ? setActualImg(actualImg - 1) : setActualImg(images.length - 1)
  }
  function nextImage(){
    actualImg + 1 >= images.length ? setActualImg(0) : setActualImg(actualImg + 1)
  }

  return(
    <motion.div initial={{opacity: 0}} animate={{opacity:1}} id={title} tabIndex="-1" aria-hidden="true" className=" flex justify-center items-center overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-full" style={{background: "rgba(26, 26, 26, 0.8)"}}>
      <div className="relative p-4 w-full max-w-2xl h-full md:h-auto">
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-neutral-800 border dark:border-neutral-600">
              ...
              <div className="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                  ...
                  <button type="button" onClick={() => openModalProj(title, status)} className=" text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
              ...
              <div className="mb-4 border-b border-gray-200 dark:border-gray-800">
                      <ul className="mx-4 flex flex-wrap -mb-px text-sm font-medium text-center" id={`myTab-${title}`} data-tabs-toggle={`myTabContent-${title}`} role="tablist">
                          <li className="mr-2" role="presentation">
                              <button onClick={() => changeContent('detalles')} className={`${contentSelected === 'detalles' ? 'hover:text-blue-600 dark:text-blue-500 dark:hover:text-blue-500 border-blue-600 dark:border-blue-500 text-blue-600' : 'hover:text-gray-600 border-transparent hover:border-gray-300 dark:hover:text-gray-300 dark:border-transparent text-gray-500 dark:text-gray-400 border-gray-100 dark:border-gray-700'} inline-block p-4 rounded-t-lg border-b-2`} id="tabpanel-tab" data-tabs-target="#details" type="button" role="tab" aria-controls="profile" aria-selected="true">
                              Detalles</button>
                          </li>
                          <li className="mr-2" role="presentation">
                              <button onClick={() => changeContent('imagenes')} className={`${contentSelected === 'imagenes' ? 'hover:text-blue-600 dark:text-blue-500 dark:hover:text-blue-500 border-blue-600 dark:border-blue-500 text-blue-600' : 'hover:text-gray-600 border-transparent hover:border-gray-300 dark:hover:text-gray-300 dark:border-transparent text-gray-500 dark:text-gray-400 border-gray-100 dark:border-gray-700'} inline-block p-4 rounded-t-lg border-b-2`} id={`images-tab-${title}`} data-tabs-target={`#images-${title}`} type="button" role="tab" aria-controls={`images-${title}`} aria-selected="false">
                              Imagenes</button>
                          </li>
                      </ul>
              </div>
                  ...
                  <div className="p-4 m-4 bg-gray-50 rounded-lg dark:bg-neutral-700" id="details" role="tabpanel" aria-labelledby="tabpanel-tab">
                          ...
                          <div className='grid sm:grid-cols-2 xs:grid-cols-1'>
                            ...
                            <div className='flex justify-center items-center'>
                              <QRCode className={`rounded-md`} value={urlWeb} />
                            </div>
                          </div>
                  </div>
                  :
                  /*Imagenes*/
                  <div className="p-4 bg-gray-50 rounded-lg dark:bg-neutral-800" id={`images-${title}`} role="tabpanel" aria-labelledby={`images-tab-${title}`}>
                    <div id={`default-carousel-${title}`} className="relative " data-carousel="static">
              ...
              <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                  <a href={urls.demo} target='_blank' rel='noreferrer' className="text-white cursor-pointer bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Ver Demo
                    <i className="fa-solid fa-globe ml-2"></i>
                  </a>
                  <a href={urls.git} target='_blank' rel='noreferrer' className="text-gray-500 cursor-pointer bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                    Github
