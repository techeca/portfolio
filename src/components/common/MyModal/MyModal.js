...
export default function MyModal({modalHandler, data}){
  ...
  return(
    <motion.div initial={{opacity: 0}} animate={{opacity:1}} id={`${data.title}-modal`} tabIndex="-1" className=" flex justify-center items-center overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-full" style={{background: "rgba(26, 26, 26, 0.8)"}}>
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-neutral-800 border dark:border-neutral-600">
              ...
              <div id={`myTabContent-${data.title}`}>
                {contentSelected === 'detalles' ?
                  ...
                  <div className="p-4 m-4 bg-gray-50 rounded-lg dark:bg-neutral-700" id={`${data.title}-details`} >
                  ...
                  :
                  /*Imagenes*/
                  <div className="p-4 bg-gray-50 rounded-lg dark:bg-neutral-800" id={`images-${data.title}`}>
                    <div id={`carousel-${data.title}`} className="relative " data-carousel="static">
                          ...
                          <div className="relative h-56 overflow-hidden rounded-lg md:h-64">
                             {data.images.length > 0 ?
                                  data.images.map((im, i) =>
                                    <div key={`img-${i}`} className={`duration-700 ease-in-out absolute inset-0 transition-all transform ${actualImg === i ? 'z-20 translate-x-0' : 'z-10 translate-x-full'}`} >
                          ...
                          {/*cambiar imagen izq*/}
                          <button type="button" onClick={() => prevImage()} className="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none">
                          ...
                          {/*cambiar imagen der*/}
                          <button type="button" onClick={() => nextImage()} className="absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none">
                              <span className="inline-flex items-center justify-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-white/30 dark:bg-gray-500/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
